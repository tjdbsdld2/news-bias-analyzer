...
def refresh_llm_config_from_env() -> None:
    ...
    USE_RESPONSE_FORMAT = (
        os.getenv("OPENROUTER_USE_RESPONSE_FORMAT", "1" if USE_RESPONSE_FORMAT else "0").strip()
        == "1"
    )
...
def normalize_analysis(analysis: Dict) -> Dict:
    ...
    for key in PERSPECTIVE_KEYS:
        normalized_vector[key] = max(0.0, min(100.0, safe_float(vector.get(key), 0.0)))
    ...
    return {
        ...
        "bias_axis": max(-100.0, min(100.0, safe_float(analysis.get("bias_axis"), 0.0))),
        "bias_strength": max(0.0, min(100.0, safe_float(analysis.get("bias_strength"), 0.0))),
        "emotionality": max(0.0, min(100.0, safe_float(analysis.get("emotionality"), 0.0))),
        "source_balance": max(0.0, min(100.0, safe_float(analysis.get("source_balance"), 50.0))),
        "evidence_quality": max(0.0, min(100.0, safe_float(analysis.get("evidence_quality"), 50.0))),
        ...
        "content_bias": {
            ...
            "axis": max(-100.0, min(100.0, safe_float(content_bias.get("axis"), 0.0))),
            "strength": max(0.0, min(100.0, safe_float(content_bias.get("strength"), 0.0))),
        ...
        "background_bias": {
            ...
            "axis": max(-100.0, min(100.0, safe_float(background_bias.get("axis"), 0.0))),
            "strength": max(0.0, min(100.0, safe_float(background_bias.get("strength"), 0.0))),
            "reasoning": safe_str(background_bias.get("reasoning")),
        },
    }


def calibrate_analysis_with_article(analysis: Dict, article: Dict) -> Dict:
    ...
    labor_keywords = ["노조", "노동조합", "노동자", "근로자", "조합원", "파업", "임금 인상", "처우 개선", "단체교섭", "부당노동행위", "노동권"]
    company_keywords = ["회사", "사측", "경영진", "기업", "사용자", "손실", "생산 차질", "경영 부담", "불법 파업", "업무 방해", "경쟁력"]
    government_keywords = ["정부", "고용노동부", "대통령", "장관", "정책", "제도", "규제", "개혁"]
    risk_keywords = ["우려", "논란", "갈등", "차질", "피해", "부담", "위기", "반발", "공백", "대란"]
    benefit_keywords = ["개선", "확대", "보호", "강화", "지원", "안정", "회복", "보장"]
    ...
    if is_labor_company_issue:
        ...
        if labor_hits > company_hits:
            ...
            vector["pro_labor"] = max(safe_float(vector.get("pro_labor"), 0.0), 85.0)
            vector["pro_welfare"] = max(safe_float(vector.get("pro_welfare"), 0.0), 65.0)
            vector["pro_regulation"] = max(safe_float(vector.get("pro_regulation"), 0.0), 60.0)
            ...
            if bias_strength < 40:
                analysis["bias_strength"] = 55.0
            vector["pro_business"] = max(safe_float(vector.get("pro_business"), 0.0), 85.0)
            vector["pro_market"] = max(safe_float(vector.get("pro_market"), 0.0), 70.0)
            vector["anti_regulation"] = max(safe_float(vector.get("anti_regulation"), 0.0), 60.0)
            vector["risk_emphasis"] = max(safe_float(vector.get("risk_emphasis"), 0.0), 75.0)
            analysis["content_bias"] = {
                "favored_side": "회사/사측",
            ...
            vector["pro_labor"] = max(safe_float(vector.get("pro_labor"), 0.0), 65.0)
            vector["pro_business"] = max(safe_float(vector.get("pro_business"), 0.0), 65.0)
            vector["risk_emphasis"] = max(safe_float(vector.get("risk_emphasis"), 0.0), 60.0)
            analysis["content_bias"] = {
                "favored_side": "양측 균형",
                "disfavored_side": "",
                "axis": 0.0,
    ...
    if risk_hits > 0:
        vector["risk_emphasis"] = max(safe_float(vector.get("risk_emphasis"), 0.0), min(100.0, 45.0 + risk_hits * 5.0))

    if benefit_hits > 0:
        vector["benefit_emphasis"] = max(safe_float(vector.get("benefit_emphasis"), 0.0), min(100.0, 45.0 + benefit_hits * 5.0))

    if emotionality < 5 and risk_hits > 0:
        analysis["emotionality"] = 10.0

    if evidence_quality < 50 and ("자료" in text or "조사" in text or "통계" in text or "발표" in text or "인용" in text):
...
def fetch_url_article(news_url: str, delay: float = 1.0) -> Dict:
    if crawl_news is not None:
        result = crawl_news(news_url=news_url, delay=delay)
    ...
    response = requests.get(news_url, timeout=20, headers={"User-Agent": "Mozilla/5.0"})
    response.raise_for_status()

    html = response.text
    title_match = re.search(r"<title[^>]*>([\s\S]*?)</title>", html, flags=re.IGNORECASE)
    title = clean_html(title_match.group(1)) if title_match else news_url
...
def build_analysis_prompt(article: Dict) -> str:
    ...
    content = clip_text(safe_str(article.get("content")), 12000)
...
def call_llm_json(system_prompt: str, user_prompt: str, temperature: float = 0.1, timeout: int = LLM_TIMEOUT) -> Dict:
    refresh_llm_config_from_env()

    if not LLM_API_KEY:
        raise ValueError("OPENROUTER_API_KEY 또는 LLM_API_KEY 환경변수가 필요합니다. PowerShell 예: $env:OPENROUTER_API_KEY='sk-or-v1-...'")

    if "openrouter.ai" not in LLM_BASE_URL:
        raise ValueError("LLM_BASE_URL은 https://openrouter.ai/api/v1 이어야 합니다.")
    ...
    if response.status_code >= 400:
        if USE_RESPONSE_FORMAT and "response_format" in json.dumps(data, ensure_ascii=False):
            ...
            retry_response = requests.post(url, headers=headers, json=retry_payload, timeout=timeout)
            retry_data = retry_response.json()
            if retry_response.status_code >= 400:
                print(json.dumps(retry_data, ensure_ascii=False, indent=2))
    ...
    if "error" in data:
        raise RuntimeError("LLM response has error: " + json.dumps(data, ensure_ascii=False))

    if "choices" not in data or not data.get("choices"):
        raise RuntimeError("LLM response has no choices: " + json.dumps(data, ensure_ascii=False))

    content = data["choices"][0].get("message", {}).get("content", "")
    if not content:
        raise RuntimeError("LLM response has no content: " + json.dumps(data, ensure_ascii=False))
...
def analyze_article(article: Dict) -> Dict:
    ...
    calibrated = calibrate_analysis_with_article(normalized, article)
    return calibrated


def analyze_url(news_url: str, delay: float = 1.0) -> Dict:
    article = fetch_url_article(news_url, delay=delay)
    return analyze_article(article)


def print_analysis_summary(result: Dict, output_path: str = "") -> None:
    print(
        json.dumps(
            {
...
def main() -> None:
    ...
    parser.add_argument("--url", required=True, help="분석할 뉴스 URL")
    parser.add_argument("--output", default="analysis_result.json", help="출력 JSON 파일")
    parser.add_argument("--crawl-delay", type=float, default=1.0)
    args = parser.parse_args()

    result = analyze_url(args.url, delay=args.crawl_delay)

    with open(args.output, "w", encoding="utf-8") as file:
        json.dump(result, file, ensure_ascii=False, indent=2)

    print_analysis_summary(result, args.output)
