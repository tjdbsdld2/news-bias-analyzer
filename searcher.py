...
def search_google_news_rss(query: str, limit: int = 3) -> list[dict]:
    ...
    try:
        with urllib.request.urlopen(search_url, timeout=RSS_TIMEOUT_SECONDS) as response:
            payload = response.read()
    except (urllib.error.URLError, TimeoutError, ValueError) as exc:
        logger.warning("Google News RSS search failed for query %r: %s", query, exc)
        return []
    except Exception as exc:
        logger.warning("Unexpected RSS search error for query %r: %s", query, exc)
        return []

    try:
        root = ET.fromstring(payload)
    except ET.ParseError as exc:
        logger.warning("Google News RSS parse failed for query %r: %s", query, exc)
        return []
