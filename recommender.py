...
def _issue_terms(issue: str) -> list[str]:
    """Extract a few usable matching terms from a structured issue label."""
    ...
    for chunk in re.split(r"/", issue):
        ...
        candidates.extend(part.strip() for part in cleaned_chunk.split() if len(part.strip()) >= 2)
...
def _query_context(article: dict | None, analysis: dict) -> tuple[list[str], str]:
    ...
    query_text_parts = [
        ...
        article.get("body", "")[:300] if isinstance(article, dict) else "",
        analysis.get("summary", "") if isinstance(analysis, dict) else "",
        analysis.get("missing_perspective", "") if isinstance(analysis, dict) else "",
        " ".join(query_tags),
    ]
...
def _build_candidates(
    ...
    for row in rows:
        ...
        if issue_score <= 0 and overlap_count <= 0:
            continue
...
def recommend_articles(article: dict | None, analysis: dict, limit: int = 3) -> dict[str, Any]:
    ...
    if curated_candidates:
        return {
            "tier": "curated",
            "heading": "검수된 다른 관점 추천 기사",
            "caption": "사람이 검수한 고품질 기사 중에서, 같은 이슈를 다른 프레임으로 다룬 기사를 우선 보여줍니다.",
    ...
    if expanded_candidates:
        return {
            "tier": "expanded",
            "heading": "자동 태깅 기반 관련 관점 기사",
            "caption": "자동 수집과 LLM 태깅으로 넓힌 추천 DB에서, 관련 이슈 태그가 겹치고 프레임이 다른 기사를 보여줍니다.",
            "articles": expanded_candidates,
        }

    return {"tier": "none", "heading": "", "caption": "", "articles": []}
